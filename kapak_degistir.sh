...
_kaydet_ayarlar() {
    { echo "VARSAYILAN_KLASOR=\"$VARSAYILAN_KLASOR\""
      ...
      echo "ALT_KLASOR_DAHIL=\"$ALT_KLASOR_DAHIL\""; } > "$CONFIG"
}
_zaten_islendi_mi() {
    local aranan="$1"
...
_durum_metni() { [ "$ALT_KLASOR_DAHIL" = "1" ] && echo "AÇIK" || echo "KAPALI"; }
_ayarlar_menu() {
    clear
    ...
    echo -e "  ${YELLOW}[3]${WHITE} Alt Klasörleri Dahil Et: ${CYAN}Aç/Kapat ${MAGENTA}(şu an: $(_durum_metni))${RESET}"
    echo -e "  ${YELLOW}[4]${WHITE} Ana Menüye Dön${RESET}"
    echo -e "${MAGENTA}──────────────────────────────────────────────────────────${RESET}"
    read -p "  Seçiminiz [1-4]: " secim
    case $secim in
        1) echo ""; read -p "  Yeni Resim Tam Yolu: " yeni_r
           if [ -f "$yeni_r" ]; then VARSAYILAN_RESIM="$yeni_r"; _kaydet_ayarlar
               echo -e "  ${GREEN}✓ Kapak güncellendi.${RESET}"
           else echo -e "  ${RED}✗ Dosya bulunamadı!${RESET}"; fi
           sleep 1; _ayarlar_menu ;;
        2) echo ""; read -p "  Yeni Klasör Tam Yolu: " yeni_k
           if [ -d "$yeni_k" ]; then VARSAYILAN_KLASOR="$yeni_k"; _kaydet_ayarlar
               echo -e "  ${GREEN}✓ Klasör güncellendi.${RESET}"
           else echo -e "  ${RED}✗ Klasör bulunamadı!${RESET}"; fi
           sleep 1; _ayarlar_menu ;;
        3) if [ "$ALT_KLASOR_DAHIL" = "1" ]; then ALT_KLASOR_DAHIL="0"
           else ALT_KLASOR_DAHIL="1"; fi
           _kaydet_ayarlar
...
_videolari_isle() {
    ...
    if ! command -v ffmpeg >/dev/null 2>&1; then
        echo -e "  ${RED}✗ HATA: ffmpeg kurulu değil! 'pkg install ffmpeg' çalıştırın.${RESET}"; sleep 3; ana_menu; return
    fi
    echo ""; read -p "  [ENTER] veya Video Klasör Yolu: " girilen_klasor
    ...
    [ "$hata_sayisi" -gt 0 ] && echo -e "  ${YELLOW}ℹ Detaylar: Ana menüden [4] seçin${RESET}"
    echo -e "${GREEN}└────────────────────────────────────────┘${RESET}"
...
ana_menu() {
    ...
    echo -e "  ${YELLOW}[3]${WHITE} Ayarlar                     ${YELLOW}(Resim / Klasör / Alt Klasör)${RESET}"
    ...
    case $secim in
        1) _videolari_isle ;;
        2) read -p "  Hafıza temizlensin mi? (e/h): " onay
        ...
        4) clear
           if [ -s "$HATA_LOG" ]; then
               echo -e "${YELLOW}── SON HATA LOGU ──${RESET}"; tail -n 60 "$HATA_LOG"
           else echo -e "  ${GREEN}✓ Kayıtlı hata yok.${RESET}"; fi
           echo ""; read -p "  Enter'a bas..." _; ana_menu ;;
        5) exit 0 ;;
        *) ana_menu ;;
    esac
}
